...
ANNUAL_MAINLAND_IMMIGRATION_RATE = 0.005 * STEPS_PER_YEAR # Example: If 0.005 was intended per original more frequent step.
...
def normalize_simulation_data(annual_species_data, annual_immigration_rate, steps_per_year):
    ...
    for species_name, config in annual_species_data.items():
        ...
        processed_config["repro_rate"] = config["annual_repro_rate"] / steps_per_year
...
class BirdAgent(mesa.Agent):
    def __init__(self, unique_id, model, species_name, initial_pos):
        ...
        self.s_data = model.species_data_processed[species_name] # Get processed data
        ...
        else: # Fallback if initial_pos is somehow out of bounds (should not happen with proper init)
            print(f"Warning: Agent {unique_id} ({species_name}) initialized at invalid pos {initial_pos}. Setting default island/habitat.")
        ...
        for move in possible_moves:
            ...
            if habitat_name == "Sea" and hasattr(self, 'target_island_direction') and self.target_island_direction:
                if len(self.target_island_direction) >= 2:
                    current_dx, current_dy = move[0] - self.pos[0], move[1] - self.pos[1]
                    similarity = (current_dx * self.target_island_direction[0] + current_dy * self.target_island_direction[1])
                    if similarity > 0: score += 2
        ...
        if self.model.habitat_map[self.pos[0], self.pos[1]] == HABITAT_TYPES["Sea"]:
            ...
            for _ in range(8):
                ...
                for r in range(1, int(self.s_data["sensing_range"])):
                    check_x, check_y = int(self.pos[0] + r * np.cos(angle)), int(self.pos[1] + r * np.sin(angle))
                    if 0 <= check_x < self.model.grid.width and 0 <= check_y < self.model.grid.height:
                        if self.model.island_map_types[check_x, check_y] == "land":
                            ...
                            if dist < min_dist: min_dist, target_island_cell = dist, (check_x, check_y)
                            break
                    else: break
            if target_island_cell:
                dx, dy = target_island_cell[0] - self.pos[0], target_island_cell[1] - self.pos[1]
                norm = np.sqrt(dx**2 + dy**2)
                if norm > 0: self.target_island_direction = (dx/norm, dy/norm)
    ...
    def move(self):
        ...
        if not valid_moves:
            self.energy -= self.s_data["move_cost_land"]; return

        if current_habitat_name == "Sea": self.sense_islands()
        preferred_moves = self.get_preferred_moves(valid_moves)
        chosen_move, cost = None, 0

        if preferred_moves:
            if current_habitat_name != "Sea":
                land_moves = [m for m in preferred_moves if self.model.habitat_map[m[0],m[1]] != HABITAT_TYPES["Sea"]]
                water_moves = [m for m in preferred_moves if self.model.habitat_map[m[0],m[1]] == HABITAT_TYPES["Sea"]]
                if land_moves and (random.random() > self.s_data["dispersal_skill"] or not water_moves): chosen_move = land_moves[0]
                elif water_moves and self.energy > self.s_data["move_cost_water"] * 3: chosen_move = water_moves[0]
                elif land_moves: chosen_move = land_moves[0]
                else: chosen_move = preferred_moves[0]
            else: # At Sea
                land_dest_moves = [m for m in preferred_moves if self.model.habitat_map[m[0],m[1]] != HABITAT_TYPES["Sea"]]
                chosen_move = land_dest_moves[0] if land_dest_moves else preferred_moves[0]
        else: chosen_move = random.choice(valid_moves)

        if chosen_move:
            dest_hab_name = get_habitat_name(self.model.habitat_map[chosen_move[0], chosen_move[1]])
            ...
            else: self.energy -= self.s_data["move_cost_land"] # Penalty
        else: self.energy -= self.s_data["move_cost_land"] # Stayed
    ...
    def reproduce(self):
        ...
        if current_habitat_name == "Sea": return
        ...
        if self.energy >= self.s_data["min_repro_energy"] and random.random() < effective_repro_rate:
            ...
            possible_spawn = self.model.grid.get_neighborhood(self.pos, moore=True, include_center=True)
            valid_land_spawn = [p for p in possible_spawn if self.model.habitat_map[p[0],p[1]] != HABITAT_TYPES["Sea"]]
            spawn_point = random.choice(valid_land_spawn) if valid_land_spawn else self.pos
            offspring = BirdAgent(self.model.next_agent_id(), self.model, self.species_name, spawn_point)
    ...
    def gain_energy(self):
        ...
        base_regen = self.s_data["base_energy_regen_land"] if current_habitat_name != "Sea" else self.s_data.get("base_energy_regen_sea", 0)
        self.energy += base_regen * regen_factor
        self.energy = min(self.energy, self.s_data["max_energy"])
        self.energy = max(0, self.energy)

    def check_mortality(self):
        if self.energy <= 0 or self.age > self.s_data["max_age"]: return True # max_age is in steps
        current_habitat_name = get_habitat_name(self.model.habitat_map[self.pos[0], self.pos[1]])
        pref_score = self.s_data["habitat_preference_score"].get(current_habitat_name, 0)
        mortality_mod = 2.0 if current_habitat_name == "Sea" else (1.5 if pref_score < 3 else 1.0)
    ...
    def step(self):
        ...
        if self.check_mortality():
            ...
            self.model.schedule.remove(self); return
        self.reproduce()
...
class GalapagosModel(mesa.Model):
    ...
    def next_agent_id(self):
        self._agent_id_counter += 1; return self._agent_id_counter
    ...
    def _load_and_process_gis(self):
        ...
        try:
            ...
            print(f"ERROR loading GIS: {e}. Continuing without islands."); return
        ...
        try:
            ...
            if gdf.empty:
                print(f"WARN: Province '{galapagos_province_name}' not found in '{province_col}'. Available: {full_gdf[province_col].unique()}. Using all features."); gdf = full_gdf
            else: print(f"Filtered for '{galapagos_province_name}'.")
        except KeyError:
            print(f"ERROR: Column '{province_col}' not found for province filter. Available: {full_gdf.columns.tolist()}. Using all features."); gdf = full_gdf

        minx, miny, maxx, maxy = gdf.total_bounds
        if gdf.empty or not all(np.isfinite([minx, miny, maxx, maxy])):
            print("ERROR: Invalid GIS bounds. Cannot process islands."); self.x_scale, self.y_scale, self.x_offset, self.y_offset = 1,1,0,0; return
        ...
        print(f"GIS Bounds: ({minx:.2f}, {miny:.2f}) to ({maxx:.2f}, {maxy:.2f}). Scale: x={self.x_scale:.2f}, y={self.y_scale:.2f}")

        for idx, feat in gdf.iterrows():
            name, geom = feat.get(self.island_name_col, f"Island_{idx}"), feat.geometry
            cells = []
            for gx in range(self.width):
                for gy in range(self.height):
                    ux, uy = (gx + 0.5) / self.x_scale + self.x_offset, (gy + 0.5) / self.y_scale + self.y_offset
                    if geom.contains(Point(ux, uy)):
                        self.island_map_types[gx, gy], self.island_map_ids[gx, gy] = "land", idx
                        cells.append((gx, gy))
            if cells:
                cent_orig = geom.centroid
                gcx = max(0, min(self.width - 1, int((cent_orig.x - self.x_offset) * self.x_scale)))
                gcy = max(0, min(self.height - 1, int((cent_orig.y - self.y_offset) * self.y_scale)))
            ...
            else: print(f"WARN: {name} (ID {idx}) resulted in 0 grid cells.")
        if not self.islands_info: print("CRITICAL WARN: No islands processed from GIS.")
        ...
        for r_val in range(self.height):
            for c_val in range(self.width):
                if self.island_map_types[c_val, r_val] == "land":
                    ...
                    for dx_val in [-1,0,1]:
                        for dy_val in [-1,0,1]:
                            if dx_val == 0 and dy_val == 0: continue
                            near_sea = False
                            for depth_val in range(1, COASTAL_ZONE_DEPTH + 1):
                                ...
                                if 0 <= ncd < self.grid.width and 0 <= nrd < self.grid.height:
                                    if self.island_map_types[ncd, nrd] == "sea": near_sea = True; break
                                else: near_sea = True; break # Out of bounds is like sea
                            if near_sea: is_coastal = True; break
                        if is_coastal: break
                    
                    if is_coastal:
                        self.habitat_map[c_val, r_val] = HABITAT_TYPES["Coastal"]
                        isl_idx = self.island_map_ids[c_val, r_val]
                        for isl in self.islands_info:
                            if isl["id"] == isl_idx: isl["coastal_cells"].append((c_val,r_val)); break
                    else:
                        isl_idx = self.island_map_ids[c_val, r_val]
                        curr_isl = next((i for i in self.islands_info if i["id"] == isl_idx), None)
                        if curr_isl and curr_isl["area_cells"] > HIGHLAND_THRESHOLD_AREA:
                            dist_c = np.sqrt((c_val - curr_isl["centroid_grid"][0])**2 + (r_val - curr_isl["centroid_grid"][1])**2)
                            rad_approx = np.sqrt(curr_isl["area_cells"] / np.pi)
                            self.habitat_map[c_val, r_val] = HABITAT_TYPES["Highland"] if dist_c < rad_approx * HIGHLAND_CORE_RATIO else HABITAT_TYPES["Scrubland"]
                        else: self.habitat_map[c_val, r_val] = HABITAT_TYPES["Scrubland"]
                else: self.habitat_map[c_val, r_val] = HABITAT_TYPES["Sea"]

    def _initialize_agents(self):
        if not self.islands_info: return
        for island in self.islands_info:
            if not island["cells_coords"]: continue
            spawn_cells = [c for c in island["cells_coords"] if self.habitat_map[c[0],c[1]] in [HABITAT_TYPES["Coastal"], HABITAT_TYPES["Scrubland"]]]
            if not spawn_cells: spawn_cells = island["cells_coords"]
            if not spawn_cells: continue
            ...
            for species_name in self.species_data_processed.keys(): # Use processed data keys
                for _ in range(self.initial_pop_per_species_per_island):
                    ...
                    agent = BirdAgent(self.next_agent_id(), self, species_name, pos)
    ...
    def _setup_datacollector(self):
        ...
        for s_name in self.species_data_processed.keys(): # Use processed data keys
            model_reporters[f"{s_name}_total"] = lambda m, s=s_name: sum(1 for a in m.schedule.agents if isinstance(a,BirdAgent) and a.species_name == s)
        if self.islands_info:
            for i_info in self.islands_info:
                i_id, i_name = i_info["id"], i_info["name"].replace(" ", "_")
                model_reporters[f"Richness_{i_name}"] = lambda m,id_val=i_id: len(set(a.species_name for a in m.schedule.agents if isinstance(a,BirdAgent) and hasattr(a,'current_island_id') and a.current_island_id == id_val))
                for s_name in self.species_data_processed.keys():
                    model_reporters[f"Pop_{i_name}_{s_name}"] = lambda m,id_val=i_id,sn=s_name: sum(1 for a in m.schedule.agents if isinstance(a,BirdAgent) and hasattr(a,'current_island_id') and a.current_island_id == id_val and a.species_name == sn)
        else: print("WARN: No islands, limited data collection.")
        self.datacollector = mesa.DataCollector(model_reporters=model_reporters)

    def apply_mainland_immigration(self):
        if not self.islands_info: return
        for island in self.islands_info:
            target_cells = island.get("coastal_cells", []) or island.get("cells_coords", [])
            ...
            for species_name in self.species_data_processed.keys():
                ...
                effective_rate = self.mainland_immigration_rate_per_step * (len(target_cells) / 50.0) # Size factor
                if random.random() < effective_rate:
                    pos = random.choice(target_cells)
                    immigrant = BirdAgent(self.next_agent_id(), self, species_name, pos)
    ...
    def step(self):
        if self.schedule.get_agent_count() > 0:
            ...
            self.schedule.step()
        else: print(f"Step {self.schedule.steps}: No agents.")
        self.datacollector.collect(self)
        if self.schedule.steps % 50 == 0: print(f"--- Step {self.schedule.steps} (Week) --- Agents: {self.schedule.get_agent_count()} ---")
...
def plot_species_area_relationship(model_data_df, islands_info_list):
    if not islands_info_list: print("SAR: No island info."); return
    ...
    for isl in islands_info_list:
        ...
        if col in model_data_df.columns:
            ...
            richness.append(avg_r); areas.append(isl["area_cells"]); names.append(isl["name"])
        else: print(f"SAR: Column {col} not found.")
    if not richness or not areas: print("SAR: Not enough data."); return
    plt.figure(figsize=(10,6)); plt.scatter(areas, richness, color='blue')
    for i, name in enumerate(names): plt.annotate(name, (areas[i], richness[i]), textcoords="offset points", xytext=(0,5), ha='center', fontsize=8)
    plt.xscale('log'); plt.yscale('log')
    plt.xlabel("Island Area (Cells) - Log"); plt.ylabel(f"Species Richness (Avg. Last {min(50,len(model_data_df))} Weeks) - Log")
    plt.title("Species-Area Relationship (SAR)"); plt.grid(True, which="both", ls="--", alpha=0.5); plt.tight_layout(); plt.show()

def plot_species_isolation_relationship(model_data_df, islands_info_list, mainland_point):
    if not islands_info_list: print("SIR: No island info."); return
    richness, distances, names = [], [], []
    for isl in islands_info_list:
        col = f"Richness_{isl['name'].replace(' ', '_')}"
        if col in model_data_df.columns:
            avg_r = model_data_df[col].tail(min(50, len(model_data_df))).mean()
            dist = np.sqrt((isl["centroid_grid"][0]-mainland_point[0])**2 + (isl["centroid_grid"][1]-mainland_point[1])**2)
            richness.append(avg_r); distances.append(dist); names.append(isl["name"])
        else: print(f"SIR: Column {col} not found.")
    if not richness or not distances: print("SIR: Not enough data."); return
    plt.figure(figsize=(10,6)); plt.scatter(distances, richness, color='green')
    for i, name in enumerate(names): plt.annotate(name, (distances[i], richness[i]), textcoords="offset points", xytext=(0,5), ha='center', fontsize=8)
    plt.xlabel("Isolation (Distance to Mainland Point)"); plt.ylabel(f"Species Richness (Avg. Last {min(50,len(model_data_df))} Weeks)")
...
if __name__ == "__main__":
    ...
    print(f"Simulation configured for {STEPS_PER_YEAR} steps (weeks) per year.")
    print(f"Total simulation duration: {SIMULATION_STEPS} weeks (approx. {SIMULATION_STEPS/STEPS_PER_YEAR:.2f} years).")
    ...
    print(f"Per-step mainland immigration rate: {mainland_immigration_rate_for_model:.6f}")
    ...
    for i in range(SIMULATION_STEPS):
        ...
        if model_instance.schedule.get_agent_count() == 0 and i > 20:
            print(f"Stopping early at week {i+1}: all agents perished."); break
    
    print("\nSimulation finished.")
    model_run_data = model_instance.datacollector.get_model_vars_dataframe()
    print("\n--- Simulation Results (Last 5 steps) ---"); print(model_run_data.tail())
    ...
    for s_name in processed_species_data_for_model.keys(): 
        col = f"{s_name}_total"
        if col in model_run_data.columns: plt.plot(model_run_data.index, model_run_data[col], label=f"{s_name} Total Pop")
    plt.xlabel("Step (Week)"); plt.ylabel("Total Population"); plt.title("Overall Species Populations Over Time")
    plt.legend(); plt.grid(True,ls=':',alpha=0.7); plt.tight_layout(); plt.show()

    if model_instance.islands_info:
        plt.figure(figsize=(12,6))
        for isl in model_instance.islands_info:
            col = f"Richness_{isl['name'].replace(' ', '_')}"
            if col in model_run_data.columns: plt.plot(model_run_data.index, model_run_data[col], label=f"{isl['name']} Richness")
        plt.xlabel("Step (Week)"); plt.ylabel("Species Richness"); plt.title("Island Species Richness Over Time")
        plt.legend(fontsize='small',ncol=2); plt.grid(True,ls=':',alpha=0.7); plt.tight_layout(); plt.show()
    ...
    else: print("Skipping SAR/SIR plots: no island data or model run data.")
